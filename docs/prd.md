...
*Precisione della chiusura.* Una Lettura registrata a posteriori è chiusa con quel che se ne sa, e l'app non riempie i buchi: il giorno, la sola annata, oppure niente. Sono tre stati distinti e nessuno dei tre si converte negli altri — una data di fine dedotta dal giorno dell'inserimento sarebbe plausibile e sbagliata, e finirebbe in una metrica annuale come se fosse un fatto. Una chiusura con la sola annata non ha un giorno, quindi non genera l'avanzamento finale automatico; una chiusura senza alcuna data non appartiene ad alcun anno. Una Lettura è chiusa quando ha un esito: le date della chiusura sono un dettaglio della chiusura, non ciò che la definisce.
...
- da leggere → letto: registra una lettura già avvenuta. Nasce una Lettura già chiusa con esito conclusione, e serve a chi arriva con una libreria da riempire, che è il caso per cui le date sono scelte dall'Utente. Non ha data di inizio: chi segna oggi un libro letto anni fa non sa quando l'ha cominciato, e dedurla produrrebbe una durata di lettura inventata.
- Da "da leggere" non si passa direttamente ad abbandonato: abbandonare un libro mai cominciato non è una cosa che si annota.
...
I libri finiti sono le Letture chiuse con esito conclusione nell'anno: due riletture concluse nello stesso anno contano due, coerentemente con il fatto che l'unità è la Lettura e non il Libro. L'anno di una Lettura è quello della sua data di fine, o la sua annata quando il giorno non si conosce; una Lettura chiusa senza alcuna data resta nello storico e non entra in nessun anno. Le pagine lette sono la somma degli incrementi datati nell'anno, più le pagine adottate delle Letture concluse nell'anno con la sola annata — quelle non hanno un avanzamento da cui contarle, entrano nel totale dell'anno ma non nella ripartizione per mese né nei giorni con lettura, e lo scarto è dichiarato accanto al numero. Autori più letti e generi principali sono classifiche di libri finiti, non di pagine, con il peso di ciascun libro ripartito tra i suoi autori e tra i suoi generi; i libri senza genere restano fuori dalla classifica dei generi, quindi la somma dei pesi dei generi è inferiore al numero di libri finiti, e lo scarto è dichiarato accanto alla classifica.
...
Le richieste stanno nell'elenco dei membri, insieme alle persone a cui si riferiscono: quelle ricevute da accettare o rifiutare, quelle inviate da revocare finché sono in attesa. È l'unico posto in cui compaiono, dato che l'app non ha notifiche; un contatore accanto a quella voce di menu segnala quante ne sono in attesa, altrimenti una richiesta potrebbe restare invisibile per sempre. (Stavano nel profilo, separate dall'elenco: la stessa relazione viveva in due pagine, e quella in cui compariva la persona era l'unica delle due che non poteva agire.) Un rifiuto riporta la relazione allo stato assente, senza traccia visibile a chi ha chiesto, e la richiesta può essere reinviata.

**Non esiste blocco, e questa è una questione aperta, non una decisione.** Diceva "non esiste blocco, coerentemente con un gruppo chiuso e a invito", e la motivazione era buona finché i membri si conoscevano tutti: fra persone invitate dalla stessa persona, un blocco è un'arma sproporzionata. Aperta l'istanza, quella coerenza non regge più — un rifiuto che non lascia traccia più una richiesta reinviabile senza limite, fra sconosciuti, è un vettore di molestia, e oggi l'app non ha alcuna difesa da opporgli. La funzione non è stata costruita e la scelta è consapevole (vedi `docs/lavoro-rimandato.md`): ciò che non si deve fare è continuare a giustificarne l'assenza con la chiusura del gruppo, che non c'è più.
...
Non è un'anagrafica sfogliabile e non dice quanti siano gli iscritti. Finché i membri erano "unità o decine" l'elenco poteva essere la lista completa in ordine alfabetico, ed è ciò che era; su un'istanza aperta la stessa lista sarebbe insieme una scansione dell'intera tabella a ogni apertura di pagina, un payload senza tetto e un censimento dei membri servito a chiunque sia autenticato. Il numero totale degli iscritti non compare da nessuna parte e non viene nemmeno calcolato: non è un dato che serva a trovare qualcuno, ed è l'unica misura della crescita dell'istanza che chiunque potrebbe leggere senza averne motivo.

Restano quindi due modi di raggiungere una persona, e nessun terzo. **Cercandola**: la ricerca per nome utente è il modo normale, corrisponde per sottostringa e tollera un errore di battitura, e restituisce un numero limitato di risultati. **Trovandola fra gli ultimi arrivati**: senza aver cercato nulla, l'elenco mostra una fetta breve dei membri più recenti — non i primi in ordine alfabetico, che sarebbero una selezione arbitraria e sempre uguale. Non esiste un "mostra i successivi": sfogliare l'anagrafica di un'istanza aperta non è un mestiere che qualcuno abbia, e chi cerca qualcuno ne sa il nome.

I gruppi che nascono da una relazione con chi guarda — le richieste ricevute, le richieste inviate, i collegamenti attivi — sono invece sempre completi e senza tetto, perché sono dati suoi e perché una richiesta nascosta da un limite non si potrebbe più né accettare né ritirare, e un collegamento nascosto renderebbe irraggiungibile una libreria.
...
- Utenti: non più limitati a unità o decine. L'ingresso resta dall'invito del Manutentore (ADR 0013) e nessuno può crearsi un account da solo, ma il numero dei membri non è più un limite di progetto su cui appoggiarsi: l'elenco membri e la ricerca vanno pensati per migliaia di nomi, non per una lista che sta in una schermata. Diceva "la crescita è limitata dal fatto stesso che ogni ingresso è un'operazione manuale", e su quella frase riposava più di quanto sembrasse — l'elenco membri era una `select` senza tetto proprio perché quella frase prometteva che la tabella sarebbe rimasta piccola.
